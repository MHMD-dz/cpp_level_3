...
void updatec(int number , vector <Sclient> client){
    ofstream file("bank_info2.txt" , ios::out) ;
    if (file)
    for (int i = 0; i < client.size(); i++){
    {
        if (i == number)
...
void output(int number , vector <Sclient> client ){
    ...
    cout << "are you sure you whant to update this account (y/n): " ;
    cin >> update ;
    if (update == 'y' | update == 'Y')
    {
        updatec(number,client);
    }
...
int main(){
    ...
    ACN = getString("Enter the acount number you whant to updatee: ");
